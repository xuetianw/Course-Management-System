    ...
    private void setupNewGameButton(Button button) {
        button.setOnClickListener(new View.OnClickListener() {
            ...
            public void onClick(View view) {
                Toast.makeText(GameActivity.this.getApplicationContext(), "new game", Toast.LENGTH_LONG).show();
                final Game game = new Game();
                String description = String.format("%s game", game_number++);
                game.setDescription(description);

                Call<Game> newGameCall = proxy.postgames(game);
                ProxyBuilder.callProxy(GameActivity.this.getApplicationContext(), newGameCall, returnedKey -> GameActivity.this.new_game_response(returnedKey));
            }
        });
    }
